...
type VideoInfoWithBlob = VideoInfo & {
  blobUrl?: string;
  loading?: boolean;
  error?: string;
};
...
export const VideosPlayer = ({
  ...
  const [videosWithBlobs, setVideosWithBlobs] = useState<VideoInfoWithBlob[]>([]);
  
  // Find the index of the first visible (not hidden) video
  const firstVisibleIdx = videosWithBlobs.findIndex(
    (video) => !hiddenVideos.includes(video.filename),
  );
  // Count of visible videos
  const visibleCount = videosWithBlobs.filter(
    (video) => !hiddenVideos.includes(video.filename),
  ).length;

  // Initialize video refs
  useEffect(() => {
    videoRefs.current = videoRefs.current.slice(0, videosWithBlobs.length);
  }, [videosWithBlobs]);

  // Function to create authenticated video URL
  const createAuthenticatedVideoUrl = (video: VideoInfo): VideoInfoWithBlob => {
    ...
    // Use HuggingFace's datasets viewer endpoint with token in URL for private datasets
    const urlWithToken = video.url.includes('?') 
      ? `${video.url}&token=${HF_TOKEN}`
      : `${video.url}?token=${HF_TOKEN}`;
    
    return {
      ...video,
      blobUrl: urlWithToken,
      loading: false,
    };
  };

  // Create authenticated video URLs on mount
  useEffect(() => {
    if (videosInfo.length > 0) {
      const results = videosInfo.map(createAuthenticatedVideoUrl);
      setVideosWithBlobs(results);
    }
  }, [videosInfo]);
  ...
  useEffect(() => {
    ...
    if (newlyUnhidden.length > 0) {
      videosWithBlobs.forEach((video, idx) => {
  ...
  }, [hiddenVideos, isPlaying, videosWithBlobs, currentTime]);
  ...
  useEffect(() => {
    ...
    const onCanPlayThrough = () => {
      videosReadyCount += 1;
      if (videosReadyCount === videosWithBlobs.length) {
    ...
    <>
      ...
      <div className="flex flex-wrap gap-x-2 gap-y-6">
        {videosWithBlobs.map((video, idx) => {
          ...
          if (video.error) {
            return (
              <div key={video.filename} className="max-w-96">
                <p className="truncate w-full rounded-t-xl bg-red-800 px-2 text-sm text-red-300">
                  {video.filename} (error)
                </p>
                <div className="bg-red-700 h-48 w-full rounded-b-xl flex items-center justify-center">
                  <span className="text-red-300 text-sm p-4">{video.error}</span>
                </div>
              </div>
            );
          }
          ...
          return (
            ...
            >
              ...
              >
                <source src={video.blobUrl || video.url} type="video/mp4" />
