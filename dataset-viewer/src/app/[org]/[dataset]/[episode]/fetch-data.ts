...
export async function getEpisodeData(
  ...
  try {
    ...
    const info = await fetch(jsonUrl, {
      ...
      if (!res.ok) {
        throw new Error(
          `Failed to fetch dataset info: ${res.status} ${res.statusText}`
        );
      }
    ...
    const arrayBuffer = await fetch(parquetUrl, {
      headers: getAuthHeaders(),
    }).then((res) => {
      if (!res.ok) {
        throw new Error(
          `Failed to fetch parquet file: ${res.status} ${res.statusText}`
        );
      }
    ...
    const chartDataGroups = chartGroups.map((group) =>
      chartData.map((row) =>
        groupRowBySuffix(pick(row, [...group, "timestamp"]))
      )
    );
